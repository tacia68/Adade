...
/* Este código é uma adaptação do driver Driver de acesso ao SmartLamp (ESP32 com Chip Serial CP2102*/
MODULE_AUTHOR("Nei <neijunior@ufam.edu.br>");
MODULE_DESCRIPTION("Driver de acesso ao adade (ESP32 com Chip Serial CP2102");
MODULE_LICENSE("GPL");
...
// Executado quando o arquivo /sys/kernel/smartlamp/{tem, ldr, hum} é lido (e.g., cat /sys/kernel/smartlamp/tem)
static ssize_t attr_show(struct kobject *sys_obj, struct kobj_attribute *attr, char *buff);
// Executado quando o arquivo /sys/kernel/smartlamp/{tem, ldr, hum} é escrito (e.g., echo "100" | sudo tee -a /sys/kernel/smartlamp/tem)
...
// Variáveis para criar os arquivos no /sys/kernel/smartlamp/{tem, ldr, hum}
static struct kobj_attribute  tem_attribute = __ATTR(tem, S_IRUGO | S_IWUSR, attr_show, attr_store);
...
static struct attribute      *attrs[]       = { &tem_attribute.attr, &ldr_attribute.attr, &hum_attribute.attr, NULL };
...
static int usb_probe(struct usb_interface *interface, const struct usb_device_id *id) {
    ...
    printk(KERN_INFO "Adade: Dispositivo conectado ...\n");

    // Cria arquivos do /sys/kernel/smartlamp/*
    sys_obj = kobject_create_and_add("adade", kernel_kobj);
...
static void usb_disconnect(struct usb_interface *interface) {
    printk(KERN_INFO "Adade: Dispositivo desconectado.\n");
...
// Exemplo de Comando:  GET_TEM 80
// Exemplo de Resposta: RES SET_TEM 26
static int usb_send_cmd(char *cmd, int param) {
    ...
    long resp_number = -1;                  // Número retornado pelo dispositivo (e.g., valor do tem, valor do ldr)

    printk(KERN_INFO "Adade: Enviando comando: %s\n", cmd);
    ...
    while (retries > 0) {
	    ...
	    if (ret) {
		printk(KERN_ERR "Adade: Erro de codigo %d ao enviar comando!\n", ret);
		return -1;
	    }
		...
		if (ret) {
		    printk(KERN_ERR "Adade: Erro ao ler dados da USB (tentativa %d). Codigo: %d\n", ret, retries--);
		    continue;
		}
        ...
        for (i=0; i<actual_size; i++) {
            ...
            if (usb_in_buffer[i] == '\n') {  // Temos uma linha completa
                ...
                printk(KERN_INFO "Adade: Recebido uma linha: '%s'\n", recv_line);
                ...
                if (!strncmp(recv_line, resp_expected, strlen(resp_expected))) {
                    printk(KERN_INFO "Adade: Linha eh resposta para %s! Processando ...\n", cmd);
                    ...
                    printk(KERN_INFO "Adade: Nao eh resposta para %s! Erro retornando -1\n", cmd, retries--);
...
static ssize_t attr_show(struct kobject *sys_obj, struct kobj_attribute *attr, char *buff) {
    ...
    printk(KERN_INFO "Adade: Lendo %s ...\n", attr_name);

    if (!strcmp(attr_name, "tem"))
        value = usb_send_cmd("GET_TEM", -1);
    else if (!strcmp(attr_name, "ldr"))
        value = usb_send_cmd("GET_LDR", -1);
    else
        value = usb_send_cmd("GET_HUM", -1);

    sprintf(buff, "%d\n", value);                   // Cria a mensagem com o valor do TEM, ldr ou hum
    return strlen(buff);
}
...
static ssize_t attr_store(struct kobject *sys_obj, struct kobj_attribute *attr, const char *buff, size_t count) {
    ...
    if (ret) {
        printk(KERN_ALERT "Adade: valor de %s invalido.\n", attr_name);
        return -EACCES;
    }

    printk(KERN_INFO "Adade: Setando %s para %ld ...\n", attr_name, value);

    if (!strcmp(attr_name, "tem"))
        printk(KERN_ALERT "Adade: o valor de temperatura eh apenas para leitura.\n");
    else if (!strcmp(attr_name, "hum"))
        printk(KERN_ALERT "Adade: o valor de humidade eh apenas para leitura.\n");
    else {
        printk(KERN_ALERT "Adade: o valor do ldr (sensor de luz) eh apenas para leitura.\n");
        return -EACCES;
    }
    if (ret < 0) {
        printk(KERN_ALERT "Adade: erro ao setar o valor do %s.\n", attr_name);
        return -EACCES;
    }
